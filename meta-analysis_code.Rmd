...
```
```{r, esdispersaloverall}

d_dispersal_overall <- read.csv("outputs/overall_dispersal_chisq_boot.csv")
m_dispersal.overall <- rma(yi = chisq, sei = se, data = subset(d_dispersal_overall, species != "Tetranychus.cinnabarinus_PT"))
forest(m_dispersal.overall, slab = subset(d_dispersal_overall, species != "Tetranychus.cinnabarinus_PT")$species, addcred = TRUE)
...
m_dispersalOH <- rma(yi = OR, sei = se, mods = ~Taxonomic.group, data = subset(d_dispmeta_OH, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus"))
m_dispersalOL <- rma(yi = OR, sei = se, mods = ~Taxonomic.group+Dispersal.assay.duration, data = subset(d_dispmeta_OL, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus"))

```

```{r dispersalcontrforest}

# repeating the models because overall effect is removed from forest plots when moderators are included
m_dispersalOH <- rma(yi = OR, sei = se, data = subset(d_dispmeta_OH, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus"))
m_dispersalOL <- rma(yi = OR, sei = se, data = subset(d_dispmeta_OL, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus"))

forest(m_dispersalOH, slab = subset(d_OR_dispersal_OH, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")$species, addcred = TRUE)
forest(m_dispersalOL, slab = subset(d_OR_dispersal_OL, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")$species, addcred = TRUE)
...
ggplot(subset(d_fitdispcontr, species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus"), aes(x = -es, y = OR, col = HILO, fill = HILO))+
  geom_point()+
...
# same species from different labs look like different species - need to correct
m_fitdispcontr <- lmer(OR ~ es+HILO + (1|Taxonomic.group/species), data = na.omit(subset(d_fitdispcontr, select = c(OR, es, HILO, Taxonomic.group, species), species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")))
summary(m_fitdispcontr)
drop1(m_fitdispcontr, test = "Chisq")


m_fitdispcontr_OH <- lmer(OR ~ es + (1|Taxonomic.group/study), data = na.omit(subset(d_fitdispcontr, select = c(OR, es, Taxonomic.group, study), HILO == "hot" & species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")))
summary(m_fitdispcontr_OH)
drop1(m_fitdispcontr_OH, test = "Chisq")



m_fitdispcontr_OL <- lmer(OR ~ es + (1|Taxonomic.group/species), data = na.omit(subset(d_fitdispcontr, select = c(OR, es, Taxonomic.group, species), HILO == "cold" & species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")))
summary(m_fitdispcontr_OL)
drop1(m_fitdispcontr_OL, test = "Chisq")

d_sub <- subset(d_fitdispcontr, HILO == "hot" & species != "Tetranychus.turkestani" & species != "Tetranychus.cinnabarinus")
d_sub$V_dispersal <- with(d_sub, I(se_dispersal)^2)
m_fitdispcontr_OH_meta <- rma.mv(yi = OR, V = V_dispersal, random = list(~ 1 | study), mods = ~Taxonomic.group + es, data = na.omit(subset(d_sub, select = c(OR, V_dispersal, study, Taxonomic.group, es))))
