...
opt.temp = 24 #value of low temperature treatment in Celsius
medium.high.temp = 30 #value of optimal temperature treatment in Celsius
...
## phase1 : Temp.treatment (levels = opt, medium.high, high), Temp.value (in degrees C), random effects (name as appropriate), Fitness
...
#### Temp.treatment (levels = opt, medium.high, high), Temp.value (in degrees C), random effects (name as appropriate), Num.disp, Num.nondisp (or alternatively, Num.disp and total N)
...
#### Temp.treatment (levels = opt, medium.high, high), Temp.value (in degrees C), random effects (name as appropriate), Dispersed (0/1)
...
fitness.formula <- Fitness ~ temp_treatment + (1 | plate)
...
dispersal.formula <- cbind(n_eggs_on_B, total_n_eggs - n_eggs_on_B) ~ temp_treatment + (1 | plate)
### notes
#### other.fixed.effects does not apply to every species; include as appropriate; fixed effects should be standardized
#### include as many random effects as appropriate
...
- Ras: Rascafria, Spain. Strawberry
...
However, we observed highest fitness in the Tcold treatment. The naming was changed accordingly.
...
m_fitness <- glmmTMB(
  Fitness ~ Temp.treatment + (1 | plate),
  family = tweedie(),
  data = d_phase1
)
...
output_fitness_emmeans <- emmeans(
  m_fitness,
...
m_disp <- glmmTMB(
  cbind(Num.disp, n_eggs_on_A) ~ Temp.treatment + (1 | plate),
  family = betabinomial(),
  data = d_phase2
)
...
# ANOVA 
output_disp_Tempeffect <- drop1(m_disp, test = "Chisq")

# Summary 
output_disp_summary <- summary(m_disp)

# Pairwise comparisons 
output_disp_emmeans <- emmeans(
  m_disp,
  specs = pairwise ~ Temp.treatment,
  type = "response",       # back-transforms logit to probability
  adjust = "none"
...
output_disp_contrasts <- output_disp_emmeans$contrasts %>%
  summary(infer = TRUE)
...
df_emmeans <- as.data.frame(output_disp_emmeans$emmeans)
ggplot(df_emmeans, aes(x = Temp.treatment, y = prob, col = Temp.treatment)) +
  geom_point(size = 4) +
...
diagnostic_dispersal <- simulateResiduals(fittedModel = m_disp)
...
write.table(
  output_fitness_summary$coefficients$cond,
  paste0(writing.path, "output_fitness_Summary_", Genus, ".", species, ".txt"),
...
write.table(output_disp_Tempeffect, paste(writing.path, "output_dispersal_TempMain_", Genus, ".", species, ".txt", sep = ""))

write.table(
  output_disp_summary$coefficients$cond,
  paste0(writing.path, "output_dispersal_Summary_", Genus, ".", species, ".txt"),
  sep = "\t", row.names = TRUE, quote = FALSE
)


write.table(output_disp_emmeans$emmeans, paste(writing.path, "output_dispersal_emmeans_", Genus, ".", species, ".txt", sep = ""))

write.table(output_disp_emmeans$contrasts, paste(writing.path, "output_dispersal_contrasts_", Genus, ".", species, ".txt", sep = ""))
