...
m_fitness <- glmmTMB(
  Fitness ~ Temp.treatment + (1 | population) + (1 | plate),
  family = tweedie(),
  data = d_phase1
)
...
m_disp <- glmmTMB(
  cbind(Num.disp, n_eggs_on_A) ~ Temp.treatment + (1 | population) + (1 | plate),
  family = betabinomial(),
  data = d_phase2
)
...
write.table(output_fitness_Tempeffect, paste(writing.path, "output_TempMain_", Genus, ".", species, ".txt", sep = ""))

write.table(
  output_fitness_summary$coefficients$cond,
  paste0(writing.path, "summary_cond_", Genus, ".", species, ".txt"),
  sep = "\t", row.names = TRUE, quote = FALSE
)

write.table(output_fitness_emmeans$emmeans, paste(writing.path, "output_emmeans_", Genus, ".", species, ".txt", sep = ""))

write.table(output_fitness_emmeans$contrasts, paste(writing.path, "output_contrasts_", Genus, ".", species, ".txt", sep = ""))
...
write.table(
  output_disp_summary$coefficients$zi,
