...
author: "Raquel Gaspar, Sofia Costa, Leonor R Rodrigues"
...
Genus = "Tetranychus"
species = "cinnabarinus_PT"
fitness.proxy = "Fecundity"
low.temp = 25 #value of low temperature treatment in Celsius(called it control because this temperature is not costly for them - they are adapted to it in the lab)
...
transform.description <-  "square-root" # leave as-is if not applicable /log/log10
...
fitness.formula <- "Fitness ~ Temp.treatment + (1|Female_survival)"
...
dispersal.formula <- "cbind(Num.disp, Num.nondisp) ~ Temp.treatment * Time + (1|Block)"
...
habitat <- "terrestrial"
Tvariation <- "homogeneous"
species.body.size <- 0.44 # mean body length in mm
...
- conceptualization: Leonor Rodrigues
- statistical analysis: Sofia Costa and Raquel Gaspar
- methodology (design of methodology): Leonor Rodrigues
- investigation/conduct experiment: Raquel Gaspar and Sofia Costa
- funding acquisition: Leonor Rodrigues
- writing (original draft, supplement): Leonor Rodrigues

## Acknowledgements
Juan Lopez for plant maintenance and Sofia Nunes for population maintenance. This work is funded by portuguese national funds through FCT – Fundação para a Ciência e a Tecnologia, I.P., under the project HotPest (https://doi.org/10.54499/2022.04172.PTDC) to LRR. 

## Study organism

Name and strain/origin of species: Tetranychus cinnabarinus

#Provide basic life history information (aquatic/terrestrial, mode of dispersal, lifespan, etc.). 

Terrestrial plant-feeding Acari,  cosmopolitan distribution, polyphagous, reproduce through arrhenotoky (haplodiploids), capable of diapause during winter, capable of both active and passive dispersal, lifespan between 2 and 4 weeks, capable of producing approximately 100 eggs during a lifetime.
...
Tetranychus cinnabarinus is a generalist species with a worldwide distribution range recently diverged from the well-studied T. urticae . In its natural environment, it is most active during the warmer months, with populations typically peaking in spring and summer. It inhabits plant surfaces, especially the undersides of leaves, and is commonly associated with crops, ornamentals, and weeds. In the lab, they are maintained at 25C on 14-day-old bean plants (25 ± 2°C, 60% RH, 16/8 h L/D) .


#How were organisms sourced?  

'WuSS' originated from five population replicates (the ‘iC’ populations in Rodrigues et al., 2022), themselves formed by merging six populations collected in crop fields in the region of Lisbon, Portugal, in 2013 (Zélé et al., 2018) from several host plants. The field populations were established at the University of Lisbon and maintained under standard conditions (25 ± 2°C, 60% RH, 16/8 h L/D) at the laboratory in high numbers (about 500–1000 females) inside cages containing bean plants (Phaseolus vulgaris, Fabaceae, var. Enana; Germisem Sementes Lda, Oliveira do Hospital, Portugal). The five population replicates were treated with rifampicin following the procedure described in Zélé et al. (2020) to remove Wolbachia. These replicates were merged in September 2020 forming the base population used here.

Rodrigues, L. R., Zélé, F., Santos, I., & Magalhães, S. (2022). No evidence for the evolution of mating behavior in spider mites due to Wolbachia-induced cytoplasmic incompatibility. Evolution, 76, 623–635.
Zélé, F., I. Santos, I. Olivieri, M. Weill, O. Duron, and S. Magalhães. (2018). Endosymbiont diversity and prevalence in herbivorous spider mite populations in South-Western Europe. FEMS Microbiol. Ecol. 94, fiy015
...
'WuSS' was maintained under standard conditions (25 ± 2°C, 60% RH, 16/8 h L/D) at the laboratory in high numbers (about 500–1000 females) inside cages containing 14-day-old bean plants (Phaseolus vulgaris, Fabaceae, var. Enana; Germisem Sementes Lda, Oliveira do Hospital, Portugal) ad lib. 


#Were specific individuals/phenotypes/stages selected for the experiment?  

Mated females (ca. 8 days at high temperatures - opt and high - and 10 to 11 days at control (low) temperature).
...
- Topt: 33°C; Optimal temperature for spider mites, as their generation time decreases and number of offspring increases (based on Costa et al. 2024).
- Thot: 36°C; Established as a sub-lethal high temperature (causing a toll in fertility but not so much in survival)
...
-	Replicates  
    - Specify number of experimental replicates: Tlab:32 ; Topt:32; Thigh:29 
    ...
    - Indicate if experimental blocks were used and the blocking design (number of blocks, how many replicates of each treatment per block, time/space between blocks): 
     Two identical incubators were used. Each block corresponded to a test temperature and the control temperature (Tlab) for this population. All replicates of each temperature treatment were done at the same time, amounting to 2 blocks (block 1: test Thigh + Control; block 2: Topt + Control).
    
-	Type and amount of food provided during experiment: 16Ø circles of 14 day-old bean leaves per individual, corresponding to food ad lib. Water is added to the petri dishes when they start drying. 
...
- Phenotypes measured (activity, body condition, body size, etc.) and how they were estimated: fecundity (number of eggs)
...
Squared petri dishes with a layer of water-soaked cotton will be used. In each, 12 circles (18 mm Ø) cut from 14 day-old bean leaves. Groups of 3 circles in line connected with an acetate bridge (1x3 cm) (i.e., 4 setups per petri dish were created) such that movement between the patches is possible. Fifteen females were added to the circle on the right (the source patch).  

 

-	Replicates  
    - Specify number of experimental replicates: Tlab:32 ; Topt:32; Thigh:31 
    
    - Number of strains and number of replicates per strain: 1 outbred population
    
    - Indicate if experimental blocks were used and the blocking design (number of blocks, how many replicates of each treatment per block, time/space between blocks): 
     Two identical incubators were used. Each block corresponded to a test temperature and the control temperature (Tlab) for this population. All replicates at 33 were measured at on the same block, replicates at 36C were collected across 2 blocks (block 1: Topt + Control; block 2: test Thigh + Control; block 3: test Thigh + Control;).
    
    
-	Type and amount of food provided during experiment: Three 16 mm Ø circles of 14 day-old bean leaves per setup, corresponding to food ad lib. Water is added to the petri dishes when they start drying.
...
-	Duration of dispersal assay including:   
    ...
    - duration of acclimation period: 8 to 11 days (at 25C and 33/36C respectively)  
    - timing and duration of dispersal assay: 24h, 48h, 72h (covariate "Time")
    
-	Controlled variables: food (ad lib), all females because they are the dispersing sex  
- Phenotypes measured (activity, body condition, body size, etc.) and how they were estimated: number of females in patch 1 vs patches 2 and 3 (I have nr of eggs per patch and damage as well but I do not provide here)   
...
shapiro.test(d_phase1$Fitness^2)
```
