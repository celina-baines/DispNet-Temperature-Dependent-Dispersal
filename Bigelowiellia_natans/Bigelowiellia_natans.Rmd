...
author: "Elvire Bestion, Delphine Legrand"
output:
  html_document:
    df_print: paged
editor_options:
...
opt.temp = 18 #value of optimal temperature treatment in Celsius
...
We used Bigelowiellia natans, a laboratory strain of Chlorarachniophyte originally sourced from the North Atlantic - Sargasso Sea in 1981. The strain was sourced from Samuel Barton and Gabriel Yvon-Durocher from Exeter University for the purpose of another experiment (Bestion et al 2021, doi:10.1073/pnas.2019591118), who  originally sourced it for the purpose of an experiment (Barton et al 2019, doi:10.1002/lno.11170) from the CCAP (The Culture Collection of Algae and Protozoa) and RCC (Roscoff Culture Collection), strain identification numbers:CCMP621, RCC 623. 

Bigelowiellia natans is a marine flagellate unicellular microalgae, distributed in Atlantic Islands, Sargasso Sea (Moestrup, Ø. & Sengco, M., 2001, Journal of Phycology).It is motile, but likely to also passively disperse due to its small size.

A previous experiment on the strain (Barton et al 19, doi:10.1002/lno.11170) estimated the thermal performance curve for growth of the species. The study used the Sharpe-Schoolfield equation to calculate thermal performance curve parameters, with a Topt of 26.68°C, an activation energy Ea of 0.99 eV, a deactivation energy Eh of 4.59eV characterizing the decreases in rate above Th (302.01 °K) where half the enzymes have become nonfunctional , and  lnc the rate normalized to an arbitrary reference temperature of 20°C is of -1.32.

A growth curve experiment done by the authors using 6 temperatures (14, 18, 22, 26, 30 and 34°C) allowed fitting a temperature performance curve using the Boatman 2017 model with the rTPC package in R (Boatman et al., 2017, Padfield et al., 2021) to calculate CTmax of 42.2 °C.

The strain was reared in K medium at 20°C on the long term before starting the experiment, on a 12/12 light cycle.
...
This led to choose 14, 18 and 30 °C.
The growth curve at the 6 temperatures was used to measure CTmax by fitting a temperature performance curve using the Boatman 2017 model with the rTPC package in R (Boatman et al., 2017, Padfield et al., 2021).
...
Algae were transferred to fresh K medium 6 days previous to the experiment so as to be in exponential phase of growth for the experiment. 
On the day of the experiment, 12 wells of a 96 well plate were filled with 250 ul of K medium and 10 ul of algae inoculate for each temperature. The other wells were filled with medium, and the reservoirs filled with 12 ml of pure water to limit evaporation during the experiment.
Plates were placed in PolEko SmartPro incubators at the three chosen temperatures on a 12:12 light:dark cycle.
Absorbance was measured at 665 nm to measure growth rate twice a day for 16 days with a TECAN Infinite M200 Pro reader.
...
Algae for the dispersal experiment were from the same transfer as the fitness assay.

Systems were filled with 2.5 ml of K medium and then clamped. Then 400 ul of algae inoculate was added to the "departure" patch, and algae were acclimated to the temperature in the closed systems in the incubators set at each given temperature and in light conditions for 30 minutes. After 30 minutes, systems were unclamped, and left for 3 hours for the algae to disperse. Then, after 3 hours, systems were re-clamped, and 80 ul of homogenized medium was collected from each tube (the "departure" and "arrival" patch), and placed onto multi-chambered counting slides (Kima). We took from each sample a 15 second video under dark-field macroscopy (Axio Zoom V16, Zeiss) and quantified cell densities using the BEMOVI R package. 
